...
export async function huggingfaceHandler(htmlIndex: string): Promise<string> {
    try {
        const response = await fetch('https://huggingface.co/api/models?author=SE6446');
        const data = await response.json();
        let count = 0;
        let modelList = '';
            ...
            modelWidget += `
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="card-text">${model.pipeline_tag || "No pipeline tag"}</p>
        ...
        htmlIndex = htmlIndex.replace('{{huggingface_models}}', '<p>Failed to load Hugging Face models.</p>');
    }
    return htmlIndex;
}

export function privateProjectHandler(htmlIndex: string): string {
    const privateProjects = Deno.readTextFileSync("./projects/private_projects.jsonl").split('\n').filter(line => line.trim() !== '').map(line => JSON.parse(line));
...
export async function huggingfaceGetModelCard(modelId: string): Promise<Response> {
    ...
    if (!response.ok) {
        console.error(`Failed to fetch model card for ${modelId}:`, response.statusText);
        if (response.status === 404) {
            return new Response(template.replace('{{content}}', `<h1>Model card not found for ${modelId}</h1>`), {
                headers: { "Content-Type": "text/html" },
                status: 404            
            });
        }
        return new Response(template.replace('{{content}}', `202 Accepted, ${response.status} ${response.statusText}`), {
            headers: { "Content-Type": "text/html" },
            status: 207            
        });
    } else {
        const markdown = await response.text().then(text=> text.replace(/---.*?---/s, "").trim()); // Remove metadata from the top of the README
...
export async function spacesHandler(htmlIndex: string): Promise<string> {
    ...
    try {
        ...
        for (const space of data) {
            ...
            name = name.replace("_", "-");
            name = name.replace("/", "-");
            let spaceWidget:string = `<div class="row"><iframe src="https://${name}.hf.space"
	        frameborder="0"
	        width="850"
	        height="450"
            ></iframe>
        ...
        htmlIndex = htmlIndex.replace('{{spaces}}', '<p>Failed to load Hugging Face spaces.</p>');
    }
    return htmlIndex;
